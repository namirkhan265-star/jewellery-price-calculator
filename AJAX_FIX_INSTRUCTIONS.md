# AJAX Live Calculator Fix

## Instructions

Replace the `ajax_calculate_live_price()` function in `/includes/class-jpc-product-meta.php` (starting around line 140) with the code below:
...
```php
    ...
    public function ajax_calculate_live_price() {
        ...
        $extra_field_1 = floatval($_POST['extra_field_1'] ?? 0);
        $extra_field_2 = floatval($_POST['extra_field_2'] ?? 0);
        $extra_field_3 = floatval($_POST['extra_field_3'] ?? 0);
        $extra_field_4 = floatval($_POST['extra_field_4'] ?? 0);
        $extra_field_5 = floatval($_POST['extra_field_5'] ?? 0);
        ...
        for ($i = 1; $i <= 5; $i++) {
            ...
            if ($enabled === 'yes' || $enabled === '1') {
                ...
                $value = floatval($_POST['extra_field_' . $i] ?? 0);
                if ($value > 0) {
                    $extra_fields[] = array(
                        'label' => $label,
                        'value' => $value
...
1. ✅ Extra Fields #1-5 now included in live calculation
2. ✅ Additional Percentage applied correctly
3. ✅ Metal-Specific GST rates used (Gold, Silver, Diamond, Platinum)
4. ✅ Discount options respected (Metals, Making, Wastage)
5. ✅ Extra field labels displayed in breakup

## Location

File: `/public_html/wp-content/plugins/jewellery-price-calculator/includes/class-jpc-product-meta.php`

Find the function `ajax_calculate_live_price()` (around line 140) and replace it with the code above.
