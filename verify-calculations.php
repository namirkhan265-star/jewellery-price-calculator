...
if (!file_exists($wp_load_path)) {
    die('Error: Cannot find wp-load.php. Make sure this file is in the plugin directory.');
}

require_once($wp_load_path);

// Check if classes exist
if (!class_exists('JPC_Metals')) {
    die('Error: JPC_Metals class not found. Make sure the plugin is activated.');
}
...
try {
    ...
    if (!$metal_id) {
        throw new Exception('No metal ID found for product ' . $product_id);
    }
    
    $metal = JPC_Metals::get_by_id($metal_id);
    if (!$metal) {
        throw new Exception('Metal not found for ID: ' . $metal_id);
    }
    ...
    if (!$plugin_prices) {
        throw new Exception('Plugin calculation returned false');
    }

    // Display comparison
    echo '<h2>EXCEL vs PLUGIN Comparison</h2>';
    echo '<table>';
    echo '<tr><th>Item</th><th>Excel (Correct)</th><th>Plugin (Current)</th><th>Difference</th><th>Status</th></tr>';
    ...
    echo '<tr class="section"><td colspan="5">CALCULATION FLOW</td></tr>';

    $flow_items = array(
        'Subtotal Before Gateway' => array($subtotal_before_gateway, $plugin_prices['subtotal_before_additional']),
        "Payment Gateway ($additional_percentage%)" => array($payment_gateway_amount, $plugin_prices['additional_percentage_amount']),
        'Subtotal After Gateway' => array($subtotal_after_gateway, $plugin_prices['subtotal_after_additional']),
        "Discount ($discount_percentage%)" => array($discount_amount, $plugin_prices['discount_amount']),
        'Price After Discount' => array($price_after_discount, $plugin_prices['subtotal_after_discount']),
        "GST on Full ($gst_percentage%)" => array($gst_on_full, $plugin_prices['gst_on_full']),
        "GST on Discounted ($gst_percentage%)" => array($gst_on_discounted, $plugin_prices['gst_on_discounted']),
    );

    foreach ($flow_items as $label => $values) {
        $diff = $values[1] - $values[0];
        $match = abs($diff) < 1;
        $class = $match ? 'correct' : 'wrong';
        echo "<tr class='$class'>";
        echo "<td><strong>$label</strong></td>";
        echo "<td><strong>₹" . number_format($values[0], 2) . "</strong></td>";
        echo "<td><strong>₹" . number_format($values[1], 2) . "</strong></td>";
        echo "<td><strong>₹" . number_format($diff, 2) . "</strong></td>";
        echo "<td>" . ($match ? '✓' : '✗') . "</td>";
        echo "</tr>";
    }

    echo '<tr class="section"><td colspan="5">FINAL PRICES</td></tr>';

    $regular_diff = $plugin_prices['regular_price'] - $regular_price_correct;
    $sale_diff = $plugin_prices['sale_price'] - $sale_price_correct;
    $regular_match = abs($regular_diff) < 1;
    $sale_match = abs($sale_diff) < 1;

    echo "<tr class='" . ($regular_match ? 'correct' : 'wrong') . "'><td><strong>Regular Price</strong></td><td><strong>₹" . number_format($regular_price_correct, 2) . "</strong></td><td><strong>₹" . number_format($plugin_prices['regular_price'], 2) . "</strong></td><td><strong>₹" . number_format($regular_diff, 2) . "</strong></td><td>" . ($regular_match ? '✓' : '✗') . "</td></tr>";

    echo "<tr class='" . ($sale_match ? 'correct' : 'wrong') . "'><td><strong>Sale Price</strong></td><td><strong>₹" . number_format($sale_price_correct, 2) . "</strong></td><td><strong>₹" . number_format($plugin_prices['sale_price'], 2) . "</strong></td><td><strong>₹" . number_format($sale_diff, 2) . "</strong></td><td>" . ($sale_match ? '✓' : '✗') . "</td></tr>";
    ...
    echo '<tr><td>GST Percentage (from plugin)</td><td>' . $plugin_prices['gst_percentage'] . '%</td></tr>';
    echo '</table>';

    // Show Excel expected values
    echo '<h2>Excel Expected Values</h2>';
    echo '<table>';
    echo '<tr><th>Item</th><th>Value</th></tr>';
    echo '<tr><td>Subtotal (before gateway)</td><td>₹61,40,478</td></tr>';
    echo '<tr><td>Payment Gateway (2%)</td><td>₹1,22,809.56</td></tr>';
    echo '<tr><td>Subtotal (after gateway)</td><td>₹62,63,288</td></tr>';
    echo '<tr><td>Discount (30%)</td><td>-₹18,78,986.27</td></tr>';
    echo '<tr><td>Price After Discount</td><td>₹43,84,301</td></tr>';
    echo '<tr><td>GST (5%)</td><td>₹2,19,215.065</td></tr>';
    echo '<tr><td><strong>Regular Price</strong></td><td><strong>₹64,82,503</strong></td></tr>';
    echo '<tr><td><strong>Sale Price</strong></td><td><strong>₹46,03,516</strong></td></tr>';
    echo '</table>';

} catch (Exception $e) {
    echo '<div style="background: #f8d7da; padding: 20px; border: 2px solid red; margin: 20px 0;">';
